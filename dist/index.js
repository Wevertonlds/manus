...
async function createCarrossel(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(carrossel).values(data);
}
async function updateCarrossel(id, data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function createInvestimento(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(investimentos).values(data);
}
async function updateInvestimento(id, data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
